...
--Similar to Center navigation but without any the bouning cone,
--instead it considers all windows with centers to the side of it's center
sideNavigation :: Navigation2D
sideNavigation = N 2 doSideNavigation
...
-- | Implements center navigation
doSideNavigation :: Eq a => Direction2D -> Rect a -> [Rect a] -> Maybe a
doSideNavigation dir (cur, rect) winrects
  | ((w, _):_) <- onCtr' = Just w
  | otherwise            = closestOffCtr
  where
    -- The center of the current window
    ...
    -- All the points that do not coincide with the current center and which
    -- lie in the (rotated) right side.
    offCtr' = L.filter (\(_, (x, y)) -> x > 0) offCtr
